...
int mcp3008Spi::spiOpen(std::string devspi)
{
  ...
  statusVal =
      ioctl(this->spifd, SPI_IOC_WR_BITS_PER_WORD, &(this->bitsPerWord));
  if (statusVal < 0) {
  ...
  statusVal =
      ioctl(this->spifd, SPI_IOC_RD_BITS_PER_WORD, &(this->bitsPerWord));
  if (statusVal < 0) {
 ...
 * ***********************************************/
mcp3008Spi::mcp3008Spi(std::string devspi,
